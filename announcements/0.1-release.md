...
I find the concept of having a personal blog kind of pretentious--I figured I'd just write the release
...
* Should calling service methods immediately submit the request, like Retrofit does, and return `Call<{true return type}>`? 

* Should the return type of `Request<{true return type}>` be explicitly required in `service!{}`, instead of being rewritten from the declared return type (assumed to be `()` if not provided)?

* Should there be request modifiers for mapping the deserialized response to another type, executed on the executor? (This is currently provided by the `Request` type, but if the request changed to being immediately submitted then this won't be accessible.)

Suggestions are welcome. Feel free to respond either on the announcement post, or 
[on this Github issue](https://github.com/abonander/anterofit/issues/1).
